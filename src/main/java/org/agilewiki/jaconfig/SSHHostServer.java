...
import org.agilewiki.jasocket.sshd.MungeRP;
import org.agilewiki.jasocket.sshd.SSHAgent;
import org.agilewiki.jasocket.sshd.SSHServer;
...
public class SSHHostServer extends HostServer implements SSHServer {
    ...
    public void registerSshServers() {
        registerServerCommand(new InterruptableServerCommand<MungeRP>(
            ...
            public void eval(String operatorName,
                ...
                Channels.req.send(SSHHostServer.this, agentChannelManager(), new RP<TreeSet<String>>() {
                    @Override
                    public void processResponse(TreeSet<String> addresses) throws Exception {
                        ...
                        while (ita.hasNext()) {
                            ...
                            (new GetAgentChannel(address)).
                                    send(SSHHostServer.this, agentChannelManager(), new RP<AgentChannel>() {
                                        @Override
                                        public void processResponse(AgentChannel response) throws Exception {
                                            if (response == null)
                                                mungeRP.processResponse(null);
                                            else
                                                shipAgent.send(SSHHostServer.this, response, mungeRP);
                                        }
                                    });
                        }
